...
for index, row in filtered_df.iterrows():
    ...
    for commit in Repository('https://github.com/' + row['Repo Name'], from_commit=start_commit, to_commit=end_commit).traverse_commits():
        for m in commit.modified_files:
            if m.filename == row['Filename'].split('/')[-1] and (commit.hash != start_commit and commit.hash != end_commit):
...
filtered_df.to_csv('data_with_metrics_fixed_secondhalf.csv', index=False)